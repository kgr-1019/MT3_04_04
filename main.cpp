...
{
	Vector3 r{};
	float dot = 2 * Dot(input, normal);

	r = Subtract(input, Multiply(dot, normal));
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (start)
		{
			...
			ball.position.z += deltaTime * ball.velocity.z;
			...
			if (IsCollision(Sphere{ ball.position,ball.radius }, plane))
			{
				...
				ball.velocity = Add(Multiply(e, projectToNormal), movingDirection);
			}
		}
		...
		// スフィア
		DrawSphere(Sphere{ ball.position,ball.radius }, worldViewProjectionMatrix, viewportMatrix);
		// 平面
